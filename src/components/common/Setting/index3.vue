...
//获取月卡
const  handleSubmit=()=> {
	axios.post(VITE_APP_API+'/api/user/card',{"accout_type":1},http_config).then(res=>{
		if(res.status ==200){
			let data= res.data;
			if(data.code ==200){
...
<template>
  ...
  <div style="display:flex;flex-wrap:wrap;justify-content:center">
		  ...
		  <div class="card_list"  :class="[index==active? 'curr' : '']"  @click="pay(item,index)" v-for="(item,index ) in cardList"  :key="index">
			  <div>{{item.title}}</div>
